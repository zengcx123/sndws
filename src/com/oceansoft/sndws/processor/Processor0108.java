 ...
 */
@Component
public class Processor0108 implements IProcess {
	private static Logger logger = Logger.getLogger(Processor0108.class);
	@Resource
	...
	public Result process(Message msg) throws Exception  {
		...
		ATAF0108 doc = msg.getBody().getDocument(ATAF0108.class);
		...
		if (num > 0) {
			return new Result(Result.FAILURE, String.format("[结果]:失败,[类型]:ATAF0108,[编号]:%1$s,[原因]：%2$s", caseCode,
					"工单已存在！"));
		}
		// 3.生成案件编号
		...
		*/
		...
		try{
			...
			logger.info("=============解析并保存市城管工单ATAF0108成功，工单号是：==============="+caseCode);
		}catch(Exception e){
			logger.error("=============保存市城管工单ATAF0108至业务平台出错！！！，工单号是：==============="+caseCode);
			logger.error("2=====>>>"+e);
		}
		...
		// 7.生成消息
		return new Result(String.format("[结果]:成功,[类型]:ATAF0108,[编号]:%1$s", caseId));
	 ...
	 * @return
	 */
	private Map<String, Object> processData(ATAF0108 doc, String caseId, String ids, String workflowId) {
	...
	//add by yixin
	public XtCaseInfo makeCaseInfo(ATAF0108 doc,String caseId,String ids){
		...
		caseInfo.setQuestionDesc("("+doc.getAcceptMode()+"_"+doc.getAcceptSource()+mainEnt+minorEnt+")"+doc.getBJContent());
